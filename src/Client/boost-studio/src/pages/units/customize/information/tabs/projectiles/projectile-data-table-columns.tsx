...
import {type GetApiProjectilesByHash200Response} from "@/api/exvs";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import {createProjectile, deleteProjectile } from "@/api/wrapper/projectile-api";
  ...
  [
    {
      ...
      header: "Hash",
      cell: ({row}) => {
        const hash = parseFloat(row.getValue("hash"))
        const formatted = hash.toString(16).toUpperCase()
        return <div className="text-center font-medium">{hash}</div>
      },
      meta: {
        isKey: true,
      }
