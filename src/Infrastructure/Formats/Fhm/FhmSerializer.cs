...
public class FhmSerializer : IFormatSerializer<Contracts.Fhm>
{
    public Task<Contracts.Fhm> DeserializeAsync(byte[] data, CancellationToken cancellationToken)
    {
        using var kaitaiStream = new KaitaiStream(data);
        var fhm = new Contracts.Fhm(kaitaiStream);
        return Task.FromResult(fhm);
    }
    
    public Task<Contracts.Fhm> DeserializeAsync(Stream data, CancellationToken cancellationToken)
    {
    ...
    public async Task<byte[]> SerializeAsync(Contracts.Fhm fhm, CancellationToken cancellationToken)
    {
    ...
    private async Task<byte[]> SerializeFhmBodyAsync(Contracts.Fhm.FhmBody fhmBody, CancellationToken cancellationToken)
    {
        await using var fhmMetadataStream = new CustomBinaryWriter(new MemoryStream(), Endianness.BigEndian);
    
        fhmMetadataStream.WriteUint(0x46484D20); // Header
        fhmMetadataStream.WriteByteArray([0x1, 0x1]);
        fhmMetadataStream.WriteByteArray([0x0, 0x10]);
        fhmMetadataStream.WriteUint(0); // UnkC
    
        var sizePosition = fhmMetadataStream.GetPosition();
        fhmMetadataStream.WriteUint(0); // reserve
