...
public record UnpackFhmToDirectory(byte[] File, string OutputDirectory, bool MultipleFiles)
    : IRequest;

public record UnpackFhmCommand(
    byte[] File,
    bool MultipleFiles,
...
) : IRequestHandler<UnpackFhmToDirectory>, IRequestHandler<UnpackFhmCommand, byte[]>
{
    public async ValueTask<Unit> Handle(
        UnpackFhmToDirectory request,
        CancellationToken cancellationToken
    )
    {
        await UnpackFhmInternal(
            request.File,
            request.MultipleFiles,
    ...
    )
    {
        ...
        var archive = await compressor.CompressAsync(
            rootFhmFolder,
    ...
    )
    {
        ...
        if (multipleFiles)
        {
            ...
            while (fileStream.Position != fileStream.Length)
            {
                ...
                var fhm = await formatBinarySerializer.DeserializeAsync(
                    fileStream,
                    cancellationToken
                );
