...
public class GetHitboxWithPaginationQueryHandler(IApplicationDbContext applicationDbContext)
    : IRequestHandler<GetHitboxWithPaginationQuery, PaginatedList<HitboxDto>>
{
    public async ValueTask<PaginatedList<HitboxDto>> Handle(
        GetHitboxWithPaginationQuery request,
        CancellationToken cancellationToken
    )
    {
        var query = applicationDbContext
            .Hitboxes.Include(entity => entity.HitboxGroup)
        ...
        if (request.UnitIds?.Length > 0)
        {
            query = query
                ...
                .Where(hitbox =>
                    hitbox.HitboxGroup!.Units.Any(unit => request.UnitIds.Contains(unit.GameUnitId))
                );
        }
        ...
        return await PaginatedList<HitboxDto>.CreateAsync(
            queryableDto,
            request.Page,
            request.PerPage
        );
