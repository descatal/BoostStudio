...
public record GetProjectileWithPaginationQuery(
    ...
    uint[]? Hashes = null,
    uint[]? UnitIds = null,
    uint[]? ModelHashes = null,
    string? Search = null
) : IRequest<PaginatedList<ProjectileDto>>;

public class GetProjectileWithPaginationQueryHandler(IApplicationDbContext applicationDbContext)
    : IRequestHandler<GetProjectileWithPaginationQuery, PaginatedList<ProjectileDto>>
{
    public async ValueTask<PaginatedList<ProjectileDto>> Handle(
        GetProjectileWithPaginationQuery request,
    ...
    )
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Search))
        {
            query = query.Where(entity =>
                ...
                || (
                    ...
                    && entity.HitboxHash.ToString()!.ToLower().Contains(request.Search)
                )
            );
        }
        ...
        {
            query = query.Where(projectile =>
                projectile.UnitProjectile != null
                && request.UnitIds.Contains(projectile.UnitProjectile.GameUnitId)
            );
        }
        ...
        if (request.ModelHashes?.Length > 0)
        {
            query = query.Where(projectile => request.ModelHashes.Contains(projectile.ModelHash));
        }

        var queryableDto = ProjectileMapper.ProjectToDto(query);
        return await PaginatedList<ProjectileDto>.CreateAsync(
            queryableDto,
            request.Page,
            request.PerPage
        );
