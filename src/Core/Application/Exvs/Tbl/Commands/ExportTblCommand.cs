...
public record ExportTblCommand(PatchFileVersion[]? Versions = null, bool ReplaceStaging = false)
    : IRequest<FileInfo>;
...
) : IRequestHandler<ExportTblCommand, FileInfo>
{
    ...
    {
        var stagingDirectory = await configsRepository.GetConfig(
            ConfigKeys.StagingDirectory,
            cancellationToken
        );
        if (
            request.ReplaceStaging
            && (stagingDirectory.IsError || string.IsNullOrWhiteSpace(stagingDirectory.Value.Value))
        )
            throw new NotFoundException(
                ConfigKeys.StagingDirectory,
                stagingDirectory.FirstError.Description
            );

        var tblQuery = applicationDbContext
            .Tbl.Include(tbl => tbl.PatchFiles)
        ...
        foreach (var tblEntry in tbl)
        {
            ...
            tblBinaries.Add(
                new FileInfo(tblBinary, tblEntry.Id.ToString(), MediaTypeNames.Application.Octet)
            );
            ...
            var stagingFilePath = Path.Combine(
                stagingDirectory.Value.Value,
