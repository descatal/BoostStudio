...
public record PackPsarcByPatchFilesCommand(
...
{
    public async ValueTask<Unit> Handle(PackPsarcByPatchFilesCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var patchFileVersion in request.PatchFileVersions ?? [])
        {
            ...
            var sourceDirectory = Path.Combine(stagingDirectoryConfig.Value.Value, "psarc", patchFileName);
