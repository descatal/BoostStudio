...
public class HotReloadScexCommandHandler(ILogger<HotReloadScexCommandHandler> logger)
    : IRequestHandler<HotReloadScex>
{
    public async ValueTask<Unit> Handle(HotReloadScex request, CancellationToken cancellationToken)
    {
        ...
        var rpcs3Memory = new ExternalMemory(rpcs3Process.Id);
        ...
        var scexPointer = rpcs3Memory.Read<uint>(
            (UIntPtr)(mapRegionPointer + unitPointer + 0x17358)
        );
