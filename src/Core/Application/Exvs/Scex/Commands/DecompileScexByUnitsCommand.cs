...
public record DecompileScexByUnitsCommand(uint[] UnitIds, bool ReplaceScript = false)
    : IRequest<FileInfo>;
...
) : IRequestHandler<DecompileScexByUnitsCommand, FileInfo>
{
    ...
    )
    {
        ...
        var units = await applicationDbContext
            .Units.Where(x => request.UnitIds.Contains(x.GameUnitId))
        ...
        foreach (var unit in units)
        {
            ...
            if (!File.Exists(sourceFilePath))
                throw new NotFoundException(nameof(sourceFilePath), $"{unit.NameEnglish}");
            ...
            try
            {
                var file = await mediator.Send(
                    new DecompileScexByPathCommand(
                        sourceFilePath,
                        destinationFilePath,
                        $"{unit.SnakeCaseName} - {unit.GameUnitId}.c"
                    ),
                    cancellationToken
                );
                files.Add(file);
