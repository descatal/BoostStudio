...
public class PsarcPacker(ILogger<PsarcPacker> logger) : IPsarcPacker
{
    ...
    )
    {
        ...
        try
        {
            await UnpackAsyncInternal(sourceFilePath, destinationPath, cancellationToken);
        }
        finally
        {
            // Cleanup regardless
    ...
    )
    {
        ...
        var unpackDirectory = isLinux ? Path.GetRandomFileName() : destinationPath;

        if (!Directory.Exists(destinationPath))
            Directory.CreateDirectory(destinationPath);

        var arguments =
            $"{wineArg} extract --input=\"{sourceFilePath}\" --to=\"{unpackDirectory}\"";
        ...
        logger.LogInformation(
            "Successfully unpacked psarc archive on: {destinationPath}",
            destinationPath
        );
    }
    ...
    )
    {
        ...
        {
            logger.LogInformation(
                "{sourcePath} is not a valid directory, exiting pack operation",
                sourcePath
            );
            return;
        }
        ...
        // Construct metadata xml: adding all the files to be packed.
        var fileSystemsPath = Directory
        ...
        await using var writer = XmlWriter.Create(
            tempMetadataPath,
            new XmlWriterSettings { Encoding = new UTF8Encoding(false), Async = true }
        );
        await metadataDoc.SaveAsync(writer, cancellationToken);
        writer.Close();

        logger.LogInformation(
            "Writing psarc xml metadata to: {tempMetadataPath}",
            tempMetadataPath
        );
        ...
        var isLinux = OperatingSystem.IsLinux();
        var executablePath = isLinux ? "/usr/bin/wine" : tempPsarcExePath;
        var executableArgs = isLinux ? $"\"{tempPsarcExePath}\"" : string.Empty;

        var arguments = $"{executableArgs} create --xml \"{tempMetadataPath}\"";
        logger.LogInformation("Executing psarc.exe with: {arguments}", arguments);
        ...
        logger.LogInformation(
            "Successfully created psarc archive on: {destinationPath}",
            destinationPath
        );
    }

    private async Task<string> InitializeExecutableAsync(
        string workingDirectory,
        CancellationToken cancellationToken
    )
    {
        ...
        await using var psarcResourceStream = Assembly
            .GetExecutingAssembly()
            .GetManifestResourceStream("BoostStudio.Infrastructure.Resources.psarc.exe");
