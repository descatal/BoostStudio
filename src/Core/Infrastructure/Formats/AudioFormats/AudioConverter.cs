...
public class AudioConverter(IBnsf bnsf) : IAudioConverter
{
    ...
    )
    {
        ...
        var baseFormatAudioBinary = await ConvertToBaseFormatAsync(
            audioBinary,
            sourceFormat,
            cancellationToken
        );
        ...
        return targetFormat switch
        {
            ...
            AudioFormat.Bnsf => await bnsf.ConvertFromRiffAsync(
                baseFormatAudioBinary,
                cancellationToken: cancellationToken
            ),
            _ => throw new Exception(
                $"Conversion from {sourceFormat} to {targetFormat} is currently unsupported!"
            ),
        };
    }
    ...
    /// </summary>
    /// <returns></returns>
    private static Task<byte[]> ConvertToBaseFormatAsync(
        byte[] audioBinary,
    ...
    )
    {
        return Task.FromResult(
            audioFormat switch
            {
                ...
                _ => throw new Exception(
                    $"Conversion from {audioFormat} to base format (PCM) is currently unsupported!"
                ),
            }
        );
    }
