...
internal class FFbinariesService
{
    ...
    internal static async Task<VersionInfo> GetVersionInfo(FFMpegVersions version)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            throw new FFMpegDownloaderException(
                $"Failed to get version info from {versionUri}",
                "network error"
            );
        }
        ...
        return versionInfo
            ?? throw new FFMpegDownloaderException(
                $"Failed to deserialize version info from {versionUri}",
                jsonString
            );
    }
    ...
    internal static IEnumerable<string> ExtractZipAndSave(Stream zipStream)
    {
        ...
        foreach (var entry in archive.Entries)
        {
            ...
            {
                entry.ExtractToFile(
                    Path.Combine(GlobalFFOptions.Current.BinaryFolder, entry.Name),
                    true
                );
