...
public static class IEndpointRouteBuilderExtensions
{
    ...
    )
    {
        ...
        var routeHandlerBuilder = builder
            .MapMethods(pattern, [httpVerb], handler)
            .DisableAntiforgery();
        ...
        routeHandlerBuilder.Add(endpointBuilder =>
        {
            ...
            var operationId = string.Join(
                "-", // join as slug
                routePattern
                    .PathSegments.Select(segment =>
                        string.Join(
                            " ",
                            segment.Parts.Select(part =>
                                part switch
                                {
                                    RoutePatternLiteralPart literalPart => literalPart.Content,
                                    RoutePatternParameterPart parameterPart =>
                                        $"by-{parameterPart.Name}", // converts id parameter to by-id, client to ById
