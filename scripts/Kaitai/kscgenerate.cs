...
var scriptDir = GetSourceFileDirectory();
var compilerPath = isWindows
    ? Path.Combine(scriptDir, "bin", "kaitai-struct-compiler.bat")
    : Path.Combine(scriptDir, "bin", "kaitai-struct-compiler");
...
var keyFiles = Directory.GetFileSystemEntries(path, "*.ksy", SearchOption.AllDirectories);

if (arguments.Count > 0)
{
    keyFiles = [.. keyFiles.Where(file => arguments.Any(arg => file.ToLower().Contains(arg)))];
}
...
// When ran using file based app command dotnet run kscgenerate.cs, the directory of the executing assembly is placed in a temp folder
// To get the kaitai executable relative to this script file, we need to use CallerFilePath attribute, which gives us the path of this source file
static string GetSourceFileDirectory([CallerFilePath] string path = "")
